...
public class City : MonoBehaviour
{
	...
	void Update()
	{
		...
		{
			if (Economy.GetPathLengthInDays(GameManager.currentCity.cityName, cityName) != 0)
			{
	...
	public void ProduceGoods(int days)
	{
		...
		{
			Economy.BuildingTransfersGoods(cityName, building.productType, building.productionAmountPerDay * days);
		}
	}
	...
	public void BuildNewProductionInCity(string product)
	{
		...
		{
			productType = product, productionAmountPerDay = Economy.GetAmountOfProduction(product)
		};
	...
	{
		Economy.PopulationConsumeGoods(cityName, days);
	}
}